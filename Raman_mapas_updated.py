# -*- coding: utf-8 -*-
"""
Created on Thu Jul 29 12:40:00 2021

@author: ALBERTO
"""
"""
# Fit Raman
Programa para calcular online valores de Raman, strain y dopaje de ficheros de witec.
Es necesario poner la ruta del fichero, y la geometría (filas y columnas)

---
Para subir el fichero a analizar, a la izquierda, ir a la pestaña **Archivos** y arrastrar ahí el fichero.
Es necesario copiar la ruta con doble barra en su estructura (**//**)
Una vez hecho, ve al menú **Entorno de ejecución** y selecciona *Ejecutar todas* **(Ctrl+F9)**


20210911: MODIFICO EL PROGRAMA DE ALBERTO PARA ANALIZARLO Y QUE FUNCIONE EN SPYDER

20220512: Cambio de nombre del archivo: Raman_mapas
"""
...
filas = 25 # de 0 a filas-1 van de izquierda a derecha
columnas = 25 # de 0 a columnas-1 van de arriba a abajo
fichero = "C:\\Users\\msinloz\\Desktop\\Crecimiento\\#64_x100_map.txt" # El archivo a subir a la izquierda en la pestaña "Archivo"
ancho = 5 # en micras, lo de las columnas (de arriba a abajo de la pantalla, con las columnas)

#Solo hay que cambiar estas dos rutas de archivo
#######################

folder_to_be_analyzed = r'To_be_analyzed_maps'
...
"""
deffolder = "C:\\Users\\msinloz\\Desktop\\Crecimiento"
deffilename = "C:/Users/msinloz/Desktop/Crecimiento/S254 3 mW_003_cm-1  (60,60) 90x90um.txt"
...
def generateLines(posGrange = [1570,1610],
                         initposG = 1570, initpos2D = 2680,
                         Vector0 = defVector0, slope = defSlopeT,
...
execute_extra_code = False
if execute_extra_code:
    pass
#----Calculate strain lines for graphs----
#    wG= 1594.8
#    w2D= 2689.1 
#    print("wG,W2D",wG,w2D)
#    calculateDopingEpsilon(wG,w2D)
    
    strain_list = [-0.05,-0.1,-0.15,-0.2,-0.25,-0.3]
    doping_zero_axis = 0
    posg,pos2d,strain,doping = [],[],[],[]
    print("PosG Pos2D epsilon(%) doping(cm-2)")
    for i in strain_list:
        posg_s,pos2d_s = calculatePosGPos2D(i,doping_zero_axis)
        posg.append(posg_s)
        pos2d.append(pos2d_s)
        strain.append(i)
        doping.append(doping_zero_axis)
#    posg,pos2d,strain,doping = np.transpose(np.loadtxt(deffolder+"epsilonpoints.csv",skiprows = 1,delimiter = " "))
    for i,nametosave in enumerate(strain):
        generateLines(posGrange = [posg[i],1650],initposG = posg[i],initpos2D = pos2d[i], slope = defSlopeH, filename = "epsilon{0}.txt".format(nametosave))
#----End calculations----
        
#----Calculate doping lines for graphs----
#    doping_list = [0,1e12,2e12,3e12,4e12,5e12,6e12,7e12,8e12,9e12,10e12,11e12,12e12,13e12,14e12,15e12]
#    strain_zero_axis = -0.1
#    posg,pos2d,strain,doping = [],[],[],[]
#    print("PosG Pos2D epsilon(%) doping(cm-2)")
#    for i in doping_list:
#        posg_s,pos2d_s = calculatePosGPos2D(strain_zero_axis,i)
#        posg.append(posg_s)
#        pos2d.append(pos2d_s)
#        strain.append(strain_zero_axis)
#        doping.append(i)
##    posg,pos2d,strain,doping = np.transpose(np.loadtxt(deffolder+"dopingpoints.csv",skiprows = 1,delimiter = " "))  #not needed now 
#    for i,nametosave in enumerate(doping):
#        generateLines(posGrange = [posg[i],1650],initposG = posg[i],initpos2D = pos2d[i], slope = defSlopeT, filename = "shiftdoping{0}e12.txt".format(nametosave/1e12))
#Simulated lines
#    for i,nametosave in enumerate(names):
#        generateLines(initposG = posg[i],initpos2D = pos2d[i], slope = defSlopeT, filename = "dopLinear{0}.txt".format(nametosave))
#----New way to generate lines
    strain_list = [-0.15,-0.2,-0.25,-0.3,-0.35,-0.4,-0.45]
    doping_list = [0,-1e12,-2e12,-3e12,-4e12,-5e12,-6e12,-7e12]
    #strain lines
    for strain in strain_list:
        posgmin,pos2dmin = calculatePosGPos2D(doping_list[0],strain)
        posgmax,pos2dmax = calculatePosGPos2D(doping_list[-1],strain)
        np.savetxt(deffolder+"epsilon{0}.txt".format(strain),np.array([[posgmin,pos2dmin],[posgmax,pos2dmax]]))
        print((pos2dmax-pos2dmin)/(posgmax-posgmin))
        
    for doping in doping_list:
        posgmin,pos2dmin = calculatePosGPos2D(doping,strain_list[0],)
        posgmax,pos2dmax = calculatePosGPos2D(doping,strain_list[-1])
        np.savetxt(deffolder+"doping{0}e12.txt".format(doping/1e12),np.array([[posgmin,pos2dmin],[posgmax,pos2dmax]]))
        print((pos2dmax-pos2dmin)/(posgmax-posgmin))
...
defFolder = 'C:\\Users\\msinloz\\Desktop\\Crecimiento'
deffileToLoad = 'C:\\Users\\msinloz\\Desktop\\Crecimiento\testraman.txt'
deffolderToSave = 'C:\\Users\\msinloz\\Desktop\\Crecimiento'
deffilenameToSave = 'load'
defmatrixToLoad = 'C:\\Users\\msinloz\\Desktop\\Crecimiento\\testraman.txt'
defspectrumToLoad = 'C:\\Users\\msinloz\\Desktop\\Crecimiento\\testraman.txt'
posXfile = 0
posYfile = 0
...
# # peakFwhmBoundaries = [[0.5,80], [0.5,60], [0.5,60], [0.5,80], 
# #                       [0.5,110], [0.5,80], [0.5,60]]

# # peakAmpValues = [100., 100., 0.1, 0.1, 100., 0.1, 0.1] 
# # peakAmpBoundaries = [[0.5,50000.], [0.5,50000.], [0.05,50000.], [0.05,50000.], 
...
# peakAmpBoundaries = [[0.5,2500000.], [0.5,2500000.], [0.05,2500000.], [0.05,2500000.], 
#                      [0.5,2500000.], [0.05,2500000.], [0.05,2500000.]]
# #----End For LORENTZ----
# #----For BWF----
# #peakPosValues = [1349, 1594, 1630, 2480,
# #                 2689,2960, 3253]
...
def fitPeaks(x, y, filenameToSave = deffilenameToSave, peak_type = 'lorentz',
    ...
    if remove_background:
        ...
        z , y , info = baseline_arPLS(y, ratio=1e-6, lam=1e4, niter=100, full_output=True)
...
def saveMatrixPlot(fileToLoad =deffileToLoad, micron_per_pixel = 1):
    ...
    matrixToPlot = np.loadtxt(fileToLoad)
        ...
        ########Incluido a fecha 24/03/2022 para eliminar la primera columna de los mapas que da error
    
    matrixToPlot = np.delete(matrixToPlot, 0, axis=1)
    matrixToPlot = np.delete(matrixToPlot, 0, axis=0)
...
#    print("x axis:", x_axis)
        y_axis = np.linspace(0,(matrixToPlot[:,1].size-1)*micron_per_pixel, matrixToPlot[:,0].size)
    except IndexError: #raised if matrixtoplot size is diferent to Index
        pass
#    print("y axis:", y_axis)
    figure(4)
    clf()
...
def getMatrixAndStatistics(pathtoMatrix, remove_zeros = True):
    ...
    matrix_array = np.loadtxt(pathtoMatrix)
        ...
        ########Incluido a fecha 24/03/2022 para eliminar la primera columna de los ardhivos de datos que dan error
    
    matrix_array = np.delete(matrix_array, 0, axis=1)
    matrix_array = np.delete(matrix_array, 0, axis=0)
...
##################Para lanzar con tkinter la seleccion de archivo, da el fichero y el nombre del fichero #############
def Lanzarajuste(names):

    

####Exception for three digits in ppl and ppi


    fichero, archivo = os.path.split(names)
    
    x = re.search("\d{2}(ppl)", archivo)
    y = re.search("\d{2}", x.group())
    filas = int(y.group()) -1 
    
    if filas ==-1:
        x = re.search("\d{3}(ppl)", archivo)
        y = re.search("\d{3}", x.group())
        filas = int(y.group()) -1 
    print(names)
    
    x = re.search("\d{2}(lpi)", archivo)
    y = re.search("\d{2}", x.group())
    columnas = int(y.group()) -1 
    
    if columnas ==-1:
        x = re.search("\d{3}(lpi)", archivo)
        y = re.search("\d{3}", x.group())
        columnas = int(y.group()) -1
    ...
    # filas = spectraMatrix.shape[0]
    # columnas = spectraMatrix.shape[1] # de 0 a columnas-1 van de arriba a abajo
    print(filas)
    print(columnas)
    try:
        x = re.search("\d{2}(um)", archivo)
        y = re.search("\d{2}", x.group())
        ancho = int(y.group())
    except:
        x = re.search("\d{1}(um)", archivo)
        y = re.search("\d{1}", x.group())
        ancho = int(y.group())  # en micras, lo de las columnas (de arriba a abajo de la pantalla, con las columnas)
#Para hacer el ajuste a lorentzianas
    ...
    filewithmatrix = os.path.join(fichero,archivo)
    ...
    saveallMatrixPlots(os.path.join(foldertosaveResults,"Matrix"),micron_per_px) #save plots
    
    #Para sacar el fichero de origin de strain y doping en función de posiciones
    PosGpath = os.path.join(foldertosaveResults,"Matrix",'PGfilter.mtx')
    Pos2Dpath = os.path.join(foldertosaveResults,"Matrix",'P2Dfilter.mtx')
    StrainPath = os.path.join(foldertosaveResults,"Matrix",'Strainfilter.mtx')
    DopingPath = os.path.join(foldertosaveResults,"Matrix",'Dopingfilter.mtx')
    save_filename = os.path.join(foldertosaveResults,"Matrix",'ForOrigin.txt' )
    print("\n PosG")
    pG_mat = getMatrixAndStatistics(PosGpath)
    print("\n Pos2D")
    p2D_mat = getMatrixAndStatistics(Pos2Dpath)
    print("\n strain")
    str_mat = getMatrixAndStatistics(StrainPath)
    print("\n doping")
    dop_mat = getMatrixAndStatistics(DopingPath)
    list_of_matrices = [pG_mat, p2D_mat,str_mat, dop_mat]
    file_header = "PosG(cm-1) Pos2D(cm-1) Strain(%) Doping(cm-2)"
    convertForOrigin(list_of_matrices, os.path.join(foldertosaveResults,"originFile.csv"), file_header)
    #----
    
    #Para sacar datos estadísticos adicionales por consola y a un fichero
    I2DvsIGpath = os.path.join(foldertosaveResults,"Matrix",'I2DvsIGfilter.mtx')
    IDvsIGpath = os.path.join(foldertosaveResults,"Matrix",'IDvsIGfilter.mtx')
    FW2Dpath = os.path.join(foldertosaveResults,"Matrix",'FW2Dfilter.mtx')
    FWDpath = os.path.join(foldertosaveResults,"Matrix",'FWDfilter.mtx')
    FWGpath = os.path.join(foldertosaveResults,"Matrix",'FWGfilter.mtx')
    save_filename = os.path.join(foldertosaveResults,"Matrix",'statistics.txt' )
    print("\n I2DvsIG")
    I2DvsIG_mat = getMatrixAndStatistics(I2DvsIGpath)
    print("\n IDvsIG")
    IDvsIG_mat = getMatrixAndStatistics(IDvsIGpath)
    print("\n FW2D")
    FW2D_mat = getMatrixAndStatistics(FW2Dpath)
    print("\n FWD")
    FWD_mat = getMatrixAndStatistics(FWDpath)
    print("\n FWG")
    FWG_mat = getMatrixAndStatistics(FWGpath)
    list_of_matrices = [I2DvsIG_mat, IDvsIG_mat, FW2D_mat, FWD_mat, FWG_mat]
    list_of_matrices = [I2DvsIGpath, IDvsIGpath, FW2Dpath, FWDpath, FWGpath]
    file_header = "I2DvsIG(ratio) I2DvsIG(ratio) FW2D(cm-1) FWD(cm-1) FWG(cm-1)"
    # convertForOrigin(list_of_matrices, os.path.join(foldertosaveResults,"statistics.txt"), file_header)
...
def multiplefoldersfiles():
    directorytoopen= r'To_be_analyzed_maps' #ask for a folder with multiple subfolders to open
